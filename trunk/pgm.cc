...
void PGM::getNextLine( char *buf, FILE *f ) {
...
bool PGM::read() {
	...
	if( !buffer ) {
		cerr << "Could not allocate memory for pgm data.\n";
		return false;
	}
...
bool PGM::write( char *outFile, file_format outFF ) {

	FILE *f = fopen( outFile, "w" );

	if( f == NULL ) {
		cerr << "Could not save pgm file." << endl;
		return false;
	}


	if( outFF == ffppm ) { // ppm is equal pgm?
			
		fprintf( f, "P6\n" );
		fprintf( f, "#File created by Ricardo Farias/Cristiana Bentes\n" );
		fprintf( f, "%u %u\n", dimX, dimY );
		fprintf( f, "255\n" );
		fwrite( buffer, 1, dimX*dimY, f );
...
void PGM::flip( flip_direction fd ) {
